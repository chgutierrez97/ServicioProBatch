...
import ast.servicio.probatch.message.MensajeMatarSubproceso;
import ast.servicio.probatch.message.MensajeProceso;
import ast.servicio.probatch.message.MensajeRespuesta;
import ast.servicio.probatch.message.MensajeSubprocesos;
...
public class MessageFactory {
	...
	public static Mensaje crearMensaje(String mensajeEntrada) throws MensajeErrorException {
		...
		String tipoMensajeTramaXml = tramaXml.getDocumentElement().getNodeName();

		if (tipoMensaje.AUTENTICACION.toString().equals(tipoMensajeTramaXml))
			return new MensajeAutenticacion(mensajeEntrada);
		if (tipoMensaje.VALIDAR.toString().equals(tipoMensajeTramaXml))
			return new MensajeValidacion(mensajeEntrada);
		if (tipoMensaje.PROCESO.toString().equals(tipoMensajeTramaXml))
			return new MensajeProceso(mensajeEntrada);
		if (tipoMensaje.LIMPIAR.toString().equals(tipoMensajeTramaXml))
			return new MensajeLimpiar(mensajeEntrada);
		if (tipoMensaje.ESTADO.toString().equals(tipoMensajeTramaXml))
			return new MensajeEstado(mensajeEntrada);
		if (tipoMensaje.MATAR.toString().equals(tipoMensajeTramaXml))
			return new MensajeMatar(mensajeEntrada);
		if (tipoMensaje.BUSCAR_LOG.toString().equals(tipoMensajeTramaXml))
			return new MensajeBuscarLog(mensajeEntrada);
		if (tipoMensaje.SUBPROCESO.toString().equals(tipoMensajeTramaXml))
			return new MensajeSubprocesos(mensajeEntrada);
		if (tipoMensaje.MATAR_SUBPROCESO.toString().equals(tipoMensajeTramaXml))
			return new MensajeMatarSubproceso(mensajeEntrada);
		if (tipoMensaje.HORA_AS400.toString().equals(tipoMensajeTramaXml))
			return new MensajeHoraAS400(mensajeEntrada);
		if (tipoMensaje.PROCESO_AS400.toString().equals(tipoMensajeTramaXml))
	...
	private static Document parsearMensaje(String xmlMessage) throws MensajeErrorException {
		try {
			...
			if (!validarTipoMensaje(tipo)) {
				throw new MensajeErrorException("comando no reconocido");
			}
			return tramaXml;
		} catch (Exception e) {
