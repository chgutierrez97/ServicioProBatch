...
public class MensajeValidacion extends Mensaje {
		...
		try {
			for (int i = 0; i < nodosHijos.getLength(); i++) {
				...
				if (nodo.equals("directorio")) {
					...
					// if
					// (usuarioDirectorio.equalsIgnoreCase(ServicioAgente.cfg.propertiesLanguage.getProperty("Administrators"))
					// ||
					// usuarioDirectorio.equalsIgnoreCase(ServicioAgente.cfg.propertiesLanguage.getProperty("Users")))
					// dominioDirectorio = "BUILTIN";
					...
					if (pathDirectorio.endsWith("/") || pathDirectorio.endsWith("\\")) {
						if (!pathDirectorio.substring(0, pathDirectorio.length() - 1).endsWith(":"))
							pathDirectorio = pathDirectorio.substring(0, pathDirectorio.length() - 1);
					}

					directorioAgregar = new Directorio(idDirectorio, StringEscapeUtils.escapeXml(pathDirectorio), pathDirMostrar, usuarioDominioArchivo,
							usuarioDirectorio, permisoDirectorio, claveDirectorio, dominioDirectorio);
			...
			logger.trace("Lanza excepcion en MensajeValidacion: " + e.getMessage());
	...
	private String validarEstado(String usuario, String permisos, String path, String dominio, boolean esArchivo) throws MensajeErrorException {
		// Chequeo la existencia del directorio/archivo
		if (Utils.validarExistenciaArchivo(path)) {
			// El directorio/archivo existe, chequeo los permisos
			if (OsServiceFactory.getOsService().buscarUsuarioPermisos(usuario, permisos, path, dominio)) {
				// El usuario tiene permisos en el directorio/archivo
				return "ok";
			} else {
				// El usuario no tiene permisos en el directorio/archivo
				return "error_permiso";
			}
		} else {
			// El directorio/archivo no existe, chequeo si se trata de un
	...
	public String validarPermisos() throws MensajeErrorException {
		...
		logger.debug("Validando permisos de los directorios");
		for (Directorio directorio : listaDirectorios) {
			logger.debug("Validando permisos del " + directorio.toString());
			if (directorio.getUsuarioDirectorio().equals("") || directorio.getPermisoDirectorio().equals("") || directorio.getIdDirectorio().equals("")
				...
				logger.error("Error permiso directorio");
			...
			respuesta = respuesta + "id=" + "\"" + directorio.getIdDirectorio() + "\"" + " " + "path=" + "\"" + directorio.getPathDirMostrar() + "\"" + " "
					+ "usuario=" + "\"" + directorio.getUsuarioDominioDirectorio() + "\"" + " ";
			if (estadoDirectorio.equals("error_permiso")) {
				respuesta = respuesta + "permiso=" + "\"\" " + "estado=" + "\"" + estadoDirectorio + "\"" + " ";
				logger.error("Error permiso directorio:: respuestaDeError= " + respuesta);
			...
			if (directorio.getArchivos() != null) {
				...
				for (Archivo archivo : directorio.getArchivos()) {
					logger.debug("Validando permisos del " + archivo.toString());
					if (archivo.getUsuario().equals("") || archivo.getPermiso().equals("") || archivo.getId().equals("") || archivo.getPath().equals("")
						...
						logger.error("Error permiso archivo");
					...
					if (estadoArchivo.equals("error_permiso")) {
						...
						logger.error("Error permiso archivo:: respuestaDeError= " + respuesta);
					} else {
						respuesta = respuesta + "permiso=" + "\"" + archivo.getPermiso() + "\"" + " " + "estado=" + "\"" + estadoArchivo + "\"" + ">"
								+ archivo.getNombre();
