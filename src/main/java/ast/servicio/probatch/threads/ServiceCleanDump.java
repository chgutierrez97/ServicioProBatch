	...
	public void run() {
		...
		try {
			while (!ServicioAgente.terminarThreads && !terminarEsteThread) {
				...
				if (fichero.exists()) {
					lista = ServicioAgente.getEstadoMensajes();
					synchronized (lista) {
						for (EstadoProceso estadoProceso : lista) {
//							logger.debug("Proceso [ id= " + estadoProceso.getId() + "; nombre= " + estadoProceso.getNombre() + "; pid= "
//									+ estadoProceso.getPid() + "; ts= " + estadoProceso.getTs() + "; estado= " + estadoProceso.getEstado()+"]");
//							logger.debug("verificando " + (timeActual - estadoProceso.getTs()) + " < " + intervaloBorradoSegundos);
							if (timeActual - estadoProceso.getTs() < intervaloBorradoSegundos) {
								estadoMensajes.add(estadoProceso);
							}
						}

//						logger.debug("ServiceCleanDump solicita SETEAR lista estado mensajes...");
...
//					logger.debug("ServiceCleanDump solicita lista estado mensajes...");
